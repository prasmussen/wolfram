...
var io = (function (fs, S, from, random, path) {
    var pickedIndexes = {};
    var filePaths = [];
    ...
    function loadWordListPaths(basePath) {
        ...
        return from(wordLists).select(function (wordListFileName) {
            var joinedPath = path.join(basePath, wordListFileName);
            return  joinedPath;
        }).toArray();
    }
    ...
    function getFilePathMatchingType(type) {
        ...
        var typeMatchRegex = /[\/|\\](\w*)\.\w*\./;
        var typePath = from(filePaths).singleOrDefault(function (filePath) {
            var match = filePath.match(typeMatchRegex);
            if (match && match[1] && match[1] === type) return true;
            return false;
        }, '');
        if (!typePath || typePath.length === 0) throw new Error('Found no path matching the type provided.');
        return typePath;
    }
    ...
    return {
        ...
        getLinesInFileSync: getLinesInFileSync,
        defaultBasePath: defaultBasePath
    }
}(fs, S, from, random, path));
