...
var wordFactory = (function (io, S, os) {
    ...
    function whiteListWord(word, path) {
        ...
        var words = S(fileContent).lines();
        if (from(words).contains(word)) {
            var re = new RegExp('(' + os.EOL + '){2,}');
            fileContent = S(fileContent).replace(word, '')
                                        .replace(re, os.EOL).s; //replace multiple newlines with single newline
            io.writeFile(path, fileContent);
...
module.exports = {
    ...
    whiteListWord: wordFactory.whiteListWord,
    privates: {
        io: io,
