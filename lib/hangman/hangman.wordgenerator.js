...
function pickARandomWord(type) {
    ...
    if (!wordlistByType || !(wordlistByType.length > 0)) {
        wordlistByType = from(wordlist).where(function (x) {
            return x.type === type
        }).toArray();
        typeWordlist[type] = wordlistByType;
    }
...
function printDebug(){
    console.log('wordlist.length = ' + wordlist.length + '\n' +
                'typeWordlist = ' + util.inspect(typeWordlist) + '\n' +
                'pickedWords.length = ' + pickedWords.length);
}
