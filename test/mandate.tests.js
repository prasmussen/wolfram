...
var TestHelper = function () {
    ...
    function humanize(ms) {
        ...
        var year = mom.years(), month = mom.months(), date = mom.days(), hour = mom.hours(), minutes = mom.minutes(), seconds = mom.seconds(), milliseconds = mom.milliseconds();
        var dateTimeString = '';
        if (year != 0) {
            dateTimeString += (year + ((year > 1) ? ' years' : ' year'));
        }
        if (month != 0) {
            dateTimeString += (month + ((month > 1) ? ' months' : ' month'));
        }
        if (date != 0) {
            if (dateTimeString.length > 0) dateTimeString += ' ';
            dateTimeString += (date + ((date > 1) ? ' days' : ' day'));
        }
        if (hour != 0) {
            if (dateTimeString.length > 0) dateTimeString += ' ';
            dateTimeString += (hour + ((hour > 1) ? ' hours' : ' hour'));
        }
        if (minutes != 0) {
            if (dateTimeString.length > 0) dateTimeString += ' ';
            dateTimeString += (minutes + ((minutes > 1) ? ' minutes' : ' minute'));
        }
        if (seconds != 0) {
            if (dateTimeString.length > 0) dateTimeString += ' ';
            dateTimeString += (seconds + ((seconds > 1) ? ' seconds' : ' second'));
        }
        if (milliseconds != 0) {
            if (dateTimeString.length > 0) dateTimeString += ' ';
            dateTimeString += (milliseconds + ((milliseconds > 1) ? ' milliseconds' : ' millisecond'));
        }
...
describe('mandate', function () {
    ...
    describe('parseDateTimeOddbjorn', function () {
        ...
        var testCases = [
            ...
            new helper.TestCase('20:00', moment({ h: 20, m: 20 }).toDate(), moment.duration({ h: 23, m: 50 }).asMilliseconds())
        ];
        helper.executeTestCases(testCases);
    });
    describe('_parseDateTime', function () {
        ...
        var testCases = [
            ...
            new helper.TestCase('20:00', moment({ h: 20, m: 20 }), moment.duration({ h: 23, m: 50 }).asMilliseconds())
        ];
        helper.executeTestCases(testCases);
    });
});